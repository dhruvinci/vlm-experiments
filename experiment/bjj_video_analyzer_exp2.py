...
class GeminiMultiPassAnalyzer:
    ...
    def _parse_pass1_markdown(self, text: str) -> Dict[str, Any]:
        ...
        for line in table_lines:
            ...
            if len(cells) >= 8:
                ...
                if time_match:
                    # Handle multiple formats: 8-column (old), 9-column (with labels), 11-column (with dom/qual)
                    if len(cells) >= 11:
                        # New format with dom, qual, labels columns
                        # Parse dominance: "athlete_1(4)" -> athlete_1, score 4
                        dom_match = re.match(r'(athlete_\d+)\((\d+)\)', cells[5])
                        dom_athlete = dom_match.group(1) if dom_match else cells[5]
                        dom_score = int(dom_match.group(2)) if dom_match else 3
                        ...
                        positions.append({
                            ...
                            'dominance_athlete': dom_athlete,
                            'dominance_score': dom_score,
                            'execution_quality': int(cells[6]) if cells[6].isdigit() else 3,
                            'control_quality': int(cells[7]) if cells[7].isdigit() else 3,
                            'confidence': float(cells[8]) if cells[8].replace('.', '').isdigit() else 1.0,
                            'labels': [l.strip() for l in cells[9].split(',') if l.strip()] if cells[9] not in ['-', ''] else [],
                            'notes': cells[10]
                        })
                    elif len(cells) >= 9:
                        # Format with labels column only
                        positions.append({
                            'start_time': time_match.group(1),
                            'end_time': time_match.group(2),
                            'position': cells[1],
                            ...
                            'labels': [l.strip() for l in cells[7].split(',') if l.strip()] if cells[7] not in ['-', ''] else [],
                            'notes': cells[8]
                        })
                    else:
                        # Old format without labels
                        positions.append({
                            'start_time': time_match.group(1),
                            'end_time': time_match.group(2),
                            'position': cells[1],
    ...
    def _analyze_window(self, cached_model, window_start: str, window_end: str, window_num: int, total_windows: int, action_segments: list = None) -> str:
        ...
        # Build action guidance if provided
        action_guidance = ""
        if action_segments:
            action_lines = []
            for seg in action_segments:
                action_lines.append(f"  - {seg['start_time']}-{seg['end_time']} (intensity: {seg.get('intensity', 0):.2f})")

            action_guidance = f"""
**🎯 HIGH-INTENSITY ACTION PERIODS (from optical flow analysis):**
{chr(10).join(action_lines)}

**GUIDANCE:** These time ranges show high grappling intensity. You MUST still provide CONTINUOUS COVERAGE of all time periods, but allocate MORE tokens/detail to these high-action segments with rich biomechanical analysis. For low-intensity periods, still document the position but be more concise in the Notes column.
"""
...
{prompt_template}"""
    ...
    def _deduplicate_positions(self, all_positions: list, overlap_seconds: int = 15) -> list:
        ...
        Only removes TRUE duplicates (same start, same end, same position).
        Does NOT remove overlapping but different segments.
        """
        ...
        for i, pos in enumerate(sorted_pos):
            ...
            # Check if next position is an EXACT duplicate
            if i + 1 < len(sorted_pos):
                ...
                # Only treat as duplicate if BOTH start AND end are within 2 seconds
                # AND the position type matches (more strict criteria)
                is_exact_duplicate = (
                    abs(pos_start - next_start) <= 2 and
                    abs(pos_end - next_end) <= 2 and
                    pos.get('position') == next_pos.get('position')
                )

                if is_exact_duplicate:
                    # True duplicate detected - keep the one with higher confidence
    ...
    def _filter_action_segments_for_window(self, action_segments: list, window_start: str, window_end: str) -> list:
        """Filter action segments that overlap with the given window."""
        if not action_segments:
            return []

        window_start_sec = self._time_to_seconds(window_start)
        window_end_sec = self._time_to_seconds(window_end)

        filtered = []
        for segment in action_segments:
            seg_start_sec = self._time_to_seconds(segment['start_time'])
            seg_end_sec = self._time_to_seconds(segment['end_time'])

            # Check if segment overlaps with window
            if seg_start_sec < window_end_sec and seg_end_sec > window_start_sec:
                filtered.append(segment)

        return filtered

    def pass_1_holistic(self, video_path: str, video_duration_seconds: int = 960, pass0_action_segments: list = None) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            pass0_action_segments: Optional list of action segments from Pass 0 to guide detail allocation
        """
        if self.verbose:
            print("\n=== PASS 1: HOLISTIC CONTEXT (MULTI-WINDOW) ===")
            print(f"  Model: gemini-2.5-flash (with caching)")
            if pass0_action_segments:
                print(f"  🎯 Pass 0 guidance: {len(pass0_action_segments)} action segments detected")

        start_time = time.time()

        # Initialize action segments if not provided
        if pass0_action_segments is None:
            pass0_action_segments = []

        # Upload video
        video_file = self._upload_video(video_path)

        # Create cache with video + expert coaching persona
        expert_persona = """You are a world-class BJJ black belt coach analyzing competition footage with the technical precision of John Danaher.
Provide film study-level analysis that elite athletes would use to improve their game. Focus on biomechanics, grips, weight distribution,
transitions, and the kinetic chain of movements. Use precise anatomical language and explain the HOW and WHY, not just the WHAT."""

        cache = caching.CachedContent.create(
            model='models/gemini-2.5-flash',
            system_instruction=expert_persona,
            contents=[video_file],
        ...
        for idx, (window_start, window_end) in enumerate(windows, 1):
            ...
            # Filter action segments for this window
            window_action_segments = self._filter_action_segments_for_window(
                pass0_action_segments, window_start, window_end
            )

            if self.verbose:
                action_info = f" ({len(window_action_segments)} action segments)" if window_action_segments else ""
                print(f"\n  [{progress}] Window {idx}/{len(windows)}{action_info}")

            response_text = self._analyze_window(
                cached_model, window_start, window_end, idx, len(windows), window_action_segments
            )
...
def main():
    ...
    if pass1_checkpoint.exists():
        ...
        # Pass action segments from Pass 0 to guide detail allocation
        pass1_output = analyzer.pass_1_holistic(
            args.video,
            pass0_action_segments=pass0_output.get('action_segments', [])
        )
