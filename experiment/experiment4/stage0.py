"""
Stage 0: Multi-Person CV Preprocessing for Experiment 4
Tracks two athletes with relative interaction metrics.
Uses shared dependencies from experiment/dependencies.
"""

import sys
import os
# Add experiment/dependencies to path (one level up from experiment4 folder)
...
class CVPreprocessor:
    ...
    def create_smart_segments(self, per_second_metrics: List[Dict], target_segments: int = 60) -> List[Dict]:
        ...
        for i, metrics in enumerate(sampled_metrics):
            ...
            if segment_length >= 8:
                # Standing <-> Ground transition (STRICT thresholds to avoid false positives)
                if metrics.get('athlete_1') and metrics.get('athlete_2') and prev_metrics.get('athlete_1') and prev_metrics.get('athlete_2'):
                    curr_standing_1 = metrics['athlete_1'].get('standing_probability', 0.5)
                    curr_standing_2 = metrics['athlete_2'].get('standing_probability', 0.5)
                    prev_standing_1 = prev_metrics['athlete_1'].get('standing_probability', 0.5)
                    prev_standing_2 = prev_metrics['athlete_2'].get('standing_probability', 0.5)
                    
                    # Average both athletes
                    curr_standing_avg = (curr_standing_1 + curr_standing_2) / 2
                    prev_standing_avg = (prev_standing_1 + prev_standing_2) / 2
                    
                    # Get interaction metrics
                    curr_contact = metrics.get('interaction', {}).get('contact_intensity', 0.0)
                    curr_action = metrics.get('action_score', 0.0)
                    
                    # TAKEDOWN: Clear standing -> clear ground + high contact + action spike
                    if (prev_standing_avg > 0.70 and curr_standing_avg < 0.30 and 
                        curr_contact > 0.15 and curr_action > 0.05):
                        should_segment = True
                        reason.append('takedown')
                    
                    # STANDUP: Clear ground -> clear standing + both athletes standing
                    elif (prev_standing_avg < 0.30 and curr_standing_avg > 0.70 and
                          curr_standing_1 > 0.65 and curr_standing_2 > 0.65):
                        should_segment = True
            ...
            # Rule 2: Action-based segmentation (NEW - before time-based!)
            if not should_segment and segment_length >= 8:
                action_score = metrics.get('action_score', 0.0)
                
                if action_score > 0.6:  # High action
                    if segment_length >= baseline_segment_length * 0.5:  # ~8s for high action
                        should_segment = True
                        reason.append('high_action')
                elif action_score > 0.3:  # Medium action
                    if segment_length >= baseline_segment_length * 0.75:  # ~12s for medium
                        should_segment = True
                        reason.append('medium_action')
            
            # Rule 3: Time-based segmentation (fallback, now last)
            if not should_segment:
                if avg_action < 0.10:  # Very static - longer segments
                    if segment_length >= baseline_segment_length * 1.5:  # ~24s
                        should_segment = True
                        reason.append('time_based_static')
                elif segment_length >= baseline_segment_length * 1.2:  # ~19s fallback
                    should_segment = True
            ...
            if should_segment:
                ...
                # Aggregate all CV metrics for this segment
                standing_probs = []
                contact_values = []
                entanglement_values = []
                velocity_values = []
                overlap_values = []
                distance_values = []
                dominant_counts = {1: 0, 2: 0, 0: 0}
                
                for m in segment_metrics:
                    if m.get('athlete_1'):
                        standing_probs.append(m['athlete_1'].get('standing_probability', 0.5))
                    if m.get('interaction'):
                        contact_values.append(m['interaction'].get('contact_intensity', 0.0))
                        entanglement_values.append(m['interaction'].get('limb_entanglement', 0))
                        velocity_values.append(m['interaction'].get('relative_velocity', 0.0))
                        overlap_values.append(m['interaction'].get('bbox_overlap', 0.0))
                        distance_values.append(m['interaction'].get('centroid_distance', 0.0))
                        dom = m['interaction'].get('dominant_athlete', 0)
                        dominant_counts[dom] = dominant_counts.get(dom, 0) + 1
                
                # Calculate averages
                avg_standing = np.mean(standing_probs) if standing_probs else 0.5
                avg_contact = np.mean(contact_values) if contact_values else 0.0
                ...
                avg_overlap = np.mean(overlap_values) if overlap_values else 0.0
                avg_distance = np.mean(distance_values) if distance_values else 0.0
                
                # Determine dominant athlete (who's on top most of the time)
                dominant_athlete = max(dominant_counts, key=dominant_counts.get)
                dominant_pct = dominant_counts[dominant_athlete] / len(segment_metrics) if segment_metrics else 0
                ...
                segments.append({
                    ...
                    'cv_avg_contact': round(float(avg_contact), 4),
                    'cv_avg_entanglement': round(float(avg_entanglement), 2),
                    'cv_avg_velocity': round(float(avg_velocity), 4),
                    'cv_avg_overlap': round(float(avg_overlap), 4),
                    'cv_avg_distance': round(float(avg_distance), 4),
                    'cv_dominant_athlete': dominant_athlete,
                    'cv_dominant_pct': round(float(dominant_pct), 3),
                    'cv_signals': reason
                })
                current_start = i
...
def build_anchor_lines(segments: List[Dict], mode: str) -> List[str]:
    """Build pipe-separated anchor lines based on mode."""
    if mode == "full":
        lines = ["**FORMAT** [start-end] | S/G: | PosHint: | Contact: | Entangle: | Velocity: | Dominant: | Action: | Flags:"]
    else:
        lines = ["**FORMAT** [start-end] | S/G: | PosHint: | Contact: | Action: | Flags:"]
    
    if mode == "none":
        for seg in segments:
            lines.append(f"**ANC [{seg['start_time']}-{seg['end_time']}]** | S/G: | PosHint: | Contact: | Action: | Flags:")
        return lines
    
    for seg in segments:
        sg = f"S/G:{seg['cv_avg_standing']:.2f}" if mode in ("minimal", "full") else "S/G:"
        hint = "standing" if seg['cv_avg_standing'] > 0.6 else ("ground" if seg['cv_avg_standing'] < 0.4 else "transition")
        hint = hint if mode in ("minimal", "full") else ""
        contact = f"{seg.get('cv_avg_contact', 0.0):.2f}" if mode == "full" else ""
        action = f"{seg['cv_avg_action']:.2f}" if mode in ("minimal", "full") else ""
        flags = ",".join(seg.get('cv_signals', [])) if mode in ("minimal", "full") else ""
        
        if mode == "full":
            entangle = f"{seg.get('cv_avg_entanglement', 0.0):.1f}"
            velocity = f"{seg.get('cv_avg_velocity', 0.0):.3f}"
            dominant = seg.get('cv_dominant_athlete', 0)
            dom_pct = seg.get('cv_dominant_pct', 0.0)
            dominant_str = f"A{dominant}({dom_pct:.0%})" if dominant > 0 else "neutral"
            
            lines.append(
                f"**ANC [{seg['start_time']}-{seg['end_time']}]** | {sg} | PosHint: {hint} | Contact: {contact} | Entangle: {entangle} | Velocity: {velocity} | Dominant: {dominant_str} | Action: {action} | Flags: {flags}"
            )
        else:
            lines.append(
                f"**ANC [{seg['start_time']}-{seg['end_time']}]** | {sg} | PosHint: {hint} | Contact: {contact} | Action: {action} | Flags: {flags}"
            )
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Experiment 4 Stage 0 (CV preprocessing)")
    parser.add_argument("--video_path", required=True, help="Path to video file")
    parser.add_argument("--anchors", choices=["none", "minimal", "full"], default="minimal", help="Anchor verbosity")
    parser.add_argument("--run", type=int, default=None, help="Run number (e.g., 1, 2, 3). If not specified, outputs without run suffix.")
    parser.add_argument("--force_recompute", action="store_true", help="Force recompute even if cache exists")
    args = parser.parse_args()
    
    OUT_DIR = Path(__file__).resolve().parents[2] / "outputs" / "experiment4.0"
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    
    # Run CV preprocessing
    result = run_cv_preprocessing(args.video_path, cache_dir=str(OUT_DIR), force=args.force_recompute)
    
    # Generate anchors
    segments = result.get('suggested_segments', [])
    anchor_lines = build_anchor_lines(segments, args.anchors)
    
    # Determine output filenames
    if args.run is not None:
        anchor_md_name = f"stage0_anchors_run{args.run}.md"
        anchor_json_name = f"stage0_anchors_run{args.run}.json"
    else:
        anchor_md_name = "stage0_anchors.md"
        anchor_json_name = "stage0_anchors.json"
    
    (OUT_DIR / anchor_md_name).write_text("\n".join(anchor_lines), encoding="utf-8")
    ...
    (OUT_DIR / anchor_json_name).write_text(json.dumps(anchor_json, indent=2), encoding="utf-8")
    
    print(f"\n[Stage 0] Complete!")
    print(f"  Anchors: {OUT_DIR/anchor_md_name} (mode={args.anchors})")
    print(f"  JSON: {OUT_DIR/anchor_json_name}")
