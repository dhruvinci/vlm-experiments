...
Analyze the complete video and create a position timeline. Focus on accuracy and meaningful segmentation.

TARGET: 20-80 segments for the full match (quality over quantity).
...
CRITICAL: Analyze the COMPLETE video from 0:00 to {duration}. Video MUST BE COMPLETELY ANALYSED. Verify your last segment reaches the end. 
...
def run_stage1_analysis(video_file, mode: str, client, existing_cache_name: str = None, previous_output: str = None, model_choice: str = "gemini-2.5-pro") -> str:
    ...
    if mode == "competition":
        prompt = COMPETITION_PROMPT.format(duration=duration_str) + continuation_context
    else:
...
def save_stage1_output(parsed_data: Dict, mode: str, video_path: str, run: int = None) -> None:
    """Save Stage 1v2 output to files."""
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    
    # Determine filenames with optional run suffix
    suffix = f"_run{run}" if run is not None else ""
    
    # Save raw MD (using stage1v2 prefix to distinguish from stage1)
    md_path = OUT_DIR / f"stage1v2_skeleton_{mode}{suffix}.md"
    ...
    json_path = OUT_DIR / f"stage1v2_skeleton_{mode}{suffix}.json"
    json_path.write_text(json.dumps(json_data, indent=2), encoding='utf-8')
    
    print(f"\n[Stage 1v2] Output saved:")
    print(f"  MD: {md_path}")
