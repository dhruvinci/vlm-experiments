...
STEP 2: CREATE POSITION TIMELINE

CV CHECKPOINTS (verify at these timestamps):
{cv_checkpoints}

Legend: S=Standing, G=Ground, M=Mixed, ?=Not detected | c=Contact (L/M/H) | e=Entanglement (limbs) | A1/A2/N=Dominant

These CV checkpoints are provided every 5 seconds to help you maintain temporal accuracy.
- Use them as REFERENCE POINTS to verify your segments align with reality
- If you see something different in the video than what CV suggests, TRUST YOUR VISUAL ANALYSIS
- CV may have errors (detection failures, occlusion), so your observation is primary
- These help prevent temporal drift - check that your segments match these anchors

TARGET: 30-100 segments for the full match.

OUTPUT STRUCTURE:

Line 1: **META** | context:competition | duration:MM:SS | athletes:Name1,Name2

Line 2: **FORMAT** | [time] | pos | top | ctrl | score | action | conf | reasons (1-10 tags) | focus (1-10 tags) | notes(max 120 char)
...
SEGMENT CRITICALITY: Do NOT create micro-segments (under 5 seconds) unless it's a submission finish or scoring sequence.
For static positions, use 30-60 second segments. Quality over quantity.
Each segment must describe a DISTINCT moment or change:
- If position is the same, describe what CHANGED (grip, pressure, submission attempt, defense)
- If nothing changed, describe a MICRO-TRANSITION (e.g., 'Micael shifts weight' or 'Andrew adjusts defense')
- NEVER repeat the exact same description in consecutive segments
- Every segment must have at least ONE new detail not in the previous segment
...
Example output (note the segment durations and confidence scores):
...
**SEG** | [0:00-0:52] | standing | - | 0.3 | 0-0 | 0.2 | 0.9 | collar_ties,circling,distance_management | - | Both athletes feeling each other out, collar ties, no serious takedown attempts
**SEG** | [0:52-1:28] | guard | Ryan | 0.5 | 0-0 | 0.4 | 0.8 | guard_passing_pressure,leg_drag_attempts | passing_pressure,leg_positioning | Ryan pressuring to pass, Pena using butterfly hooks and frames to maintain guard
**SEG** | [1:28-1:42] | half_guard | Ryan | 0.7 | 0-0 | 0.7 | 0.9 | pass_to_half,underhook_battle | half_guard_passing,pressure_application | Ryan secures half guard, working underhook, Pena defending with lockdown
**SEG** | [1:42-1:49] | mount | Ryan | 0.9 | 3-0 | 0.9 | 1.0 | guard_pass_completion,mount_transition,scoring | passing_mechanics,mount_entry,scoring_sequence | Ryan completes pass to mount, 3 points scored
**SEG** | [1:49-2:35] | mount | Ryan | 0.8 | 3-0 | 0.3 | 0.9 | positional_maintenance,submission_hunting | mount_control,attack_setup | Ryan maintaining mount, working for arm isolation, Pena defending frames
**SEG** | [2:35-2:48] | mount | Ryan | 0.9 | 3-0 | 0.9 | 0.8 | armbar_attempt,escape_scramble | armbar_mechanics,mount_escape_defense | Ryan attacks armbar, Pena explosive escape attempt, scramble ensues

TEMPORAL CRITICALITY: Analyze the COMPLETE video from 0:00 to {duration}.
Before finishing, verify your last segment reaches {duration}.
Do NOT stop early - continue until the video ends.
...
def run_stage1_analysis(video_file, mode: str, client, existing_cache_name: str = None, previous_output: str = None, model_choice: str = "gemini-2.5-pro") -> str:
    ...
    # Get video duration and load CV checkpoints
    try:
    ...
    # Load CV checkpoints for temporal anchoring
    cv_checkpoints = load_cv_checkpoints()
    
    # Select prompt based on mode
    if mode == "competition":
        prompt = COMPETITION_PROMPT.format(duration=duration_str, cv_checkpoints=cv_checkpoints) + continuation_context
    else:
...
def parse_stage1_output(output_text: str) -> Dict:
    """Parse pipe-separated MD output into structured data."""
    ...
    for line in lines:
        if line.startswith('**META**'):
            ...
            # Parse segment line - pipe-separated format
            parts = [p.strip() for p in line.split('|')]
            if len(parts) < 10:  # Need at least 10 fields with confidence
                continue
...
def save_stage1_output(parsed_data: Dict, mode: str, video_path: str, run: int = None) -> None:
    ...
    # Save raw MD
    md_path = OUT_DIR / f"stage1_skeleton_{mode}{suffix}.md"
    ...
    json_path = OUT_DIR / f"stage1_skeleton_{mode}{suffix}.json"
    json_path.write_text(json.dumps(json_data, indent=2), encoding='utf-8')
    
    print(f"\n[Stage 1] Output saved:")
    print(f"  MD: {md_path}")
    print(f"  JSON: {json_path}")
    print(f"  Segments: {len(parsed_data['segments'])}")
